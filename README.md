...
identity provider with custom extensions. This Customized Keycloak (_CuKe_) image based on Keycloak, which comes with
...
To build _CuKe_ image inside the GitLab pipeline look at the provided `.gitlab-ci.yml` which will have a jobs
...
This job will build the Docker custom keycloak image. It uses the artifacts created with `build:extensions`.
